function showToast(title, message, type = 'info', duration = 3500) {
    const toastComponent = document.getElementById('toast-component');
    const toastTitle = document.getElementById('toast-title');
    const toastMessage = document.getElementById('toast-message');
    const toastIcon = document.getElementById('toast-icon');

    if (!toastComponent || !toastIcon) return;

    const toastTypes = {
        success: {
            icon: '✔️', 
            classes: ['bg-green-50', 'border-green-500', 'text-green-700'],
            border: '1px solid #22c55e',
        },
        error: {
            icon: '❌', 
            classes: ['bg-red-50', 'border-red-500', 'text-red-700'],
            border: '1px solid #ef4444',
        },
        info: {
            icon: 'ℹ️', 
            classes: ['bg-blue-50', 'border-blue-500', 'text-blue-700'],
            border: '1px solid #3b82f6',
        }
    };

    const config = toastTypes[type] || toastTypes.info;

    Object.values(toastTypes).forEach(t => toastComponent.classList.remove(...t.classes));

    toastComponent.classList.add(...config.classes);
    toastComponent.style.border = config.border;
    toastIcon.textContent = config.icon;

    toastTitle.textContent = title;
    toastMessage.textContent = message;

    toastComponent.classList.remove('opacity-0', 'translate-y-64');
    toastComponent.classList.add('opacity-100', 'translate-y-0');

    setTimeout(() => {
        toastComponent.classList.remove('opacity-100', 'translate-y-0');
        toastComponent.classList.add('opacity-0', 'translate-y-64');
    }, duration);

}

